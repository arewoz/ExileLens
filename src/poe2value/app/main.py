...
WORKER_ARG = "--poe2value-worker"


def _maybe_run_worker_subprocess() -> None:
    if WORKER_ARG in sys.argv:
        from poe2value.worker import run_worker_entrypoint
...
from poe2value.branding import APP_NAME, app_icon, apply_windows_app_id
from poe2value.price_check.capture_diagnostics import CapturePhase as PriceCheckCapturePhase, log_capture_phase
from poe2value.app.instance_ipc import (
    COMMAND_ACTIVATE,
    COMMAND_QUIT,
    QUIT_ARG,
    InstanceServer,
...
from poe2value.app.logging_setup import configure_logging, install_crash_handlers, log_price_check_startup_mode
...
from poe2value.ui.tree_overlay_finetune import OverlayFineTuneDialog
...
class Poe2ValueApp:
    def __init__(self) -> None:
        ...
        if self._settings_load_error:
            from poe2value.app.settings import backup_settings_file
    ...
    def _restore_trade_penalties(self) -> None:
        ...
        from poe2value.price_check.rate_policy import shared_policy_registry
    ...
    def _on_league_selection_required(
        ...
        if not self.controller:
            return
        from poe2value.ui.league_dialog import prompt_for_league
    ...
    def _price_check_panel(self):
        ...
        if getattr(self, "_interactive_price_panel", None) is None:
            from poe2value.ui.interactive_price_check_panel import InteractivePriceCheckPanel
    ...
    def _open_legacy_refine_dialog(self) -> None:
        if not self.controller or not self.controller.has_last_price_check():
            return
        from poe2value.ui.refine_price_dialog import RefinePriceDialog
    ...
    def _on_presentation_invalidated(self) -> None:
        if self.overlay:
            from poe2value.ui.overlay_visibility import OverlayHideReason

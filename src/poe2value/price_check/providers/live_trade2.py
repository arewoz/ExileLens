...
from poe2value.price_check.comparable_funnel_diag import build_pass_funnel_snapshot
from poe2value.price_check.comparable_pricing import (
    build_band_estimate,
    confidence_reason,
...
from poe2value.price_check.live_acquisition import (
...
from poe2value.price_check.rate_policy import FETCH
from poe2value.price_check.trade2_client import Trade2Client, Trade2Error
...
def _mod_text(row: dict[str, Any]) -> str:
    if isinstance(row, str):
        from poe2value.price_check.comparable_features import strip_mod_markup
...
def _live_similarity_match(item_raw: str, query) -> bool:
    ...
    from poe2value.price_check.comparable_features import FAMILY_MATCH_THRESHOLD
...
class LiveTradeComparableProvider:
    ...
    def lookup(self, request: PriceCheckRequest) -> PriceCheckResult | None:
        if isinstance(request, CompiledPriceCheckRequest):
            from poe2value.price_check.trade2_query_validation import validate_trade2_search_body, InvalidTradeQuery
    ...
    def _h2_wait_seconds(self) -> float:
        try:
            from poe2value.price_check.rate_policy import SEARCH
    ...
    def _evaluate_hypothesis_locally(
        ...
        if self._session is None:
            return None
        from poe2value.price_check.comparable_query import listing_similarity_score
        from poe2value.price_check.market_session import neighbourhood_identity, query_fingerprint
    ...
    def _synthetic_cached_result(self, request: PriceCheckRequest, hypothesis, obs, league: str) -> PriceCheckResult:
        from poe2value.price_check.models import CurrencyBand

...
The product is presented to users as **ExileLens**. Internal identifiers that
would break compatibility if renamed (the ``poe2value`` Python package, the
``poe2value`` console script, the ``%LOCALAPPDATA%/poe2-value-overlay`` settings
directory, trade API user agents) intentionally keep their historical names —
...
def _asset_roots() -> tuple[Path, ...]:
    ...
    # src/poe2value/branding.py -> repo root

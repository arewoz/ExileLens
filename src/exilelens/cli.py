...
from exilelens.config import load_config, validate_pob_path
...
from exilelens.items.raw_input import ItemInputSource
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and argv[0] == "ops":
        from exilelens.ops.cli import main as ops_main

        return ops_main(argv[1:])

    parser = argparse.ArgumentParser(prog="exilelens")
        ...
        with Engine(config) as engine:
            ...
            if args.command == "evaluate-item":
                ...
                if getattr(args, "debug_comparison", False) and not args.json:
                    from exilelens.items.comparison_trace import format_comparison_trace
            ...
            if args.command == "market-search":
                ...
                from exilelens.market.models import MarketSearchRequest, SearchDepth
            ...
            if args.command == "gear-optimize":
                ...
                from exilelens.gear.models import GearOptimizationRequest, GearSearchPreset
                from exilelens.market.engine import run_market_search
                from exilelens.market.models import MarketSearchRequest, SearchDepth
            ...
            if args.command in {"tree-info", "next-passive", "tree-targets", "evaluate-node"}:
                ...
                from exilelens.tree.pipeline import tree_info_payload

"""Developer operations CLI: `python -m exilelens ops <command>`."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="exilelens ops")
    ...
    sub.add_parser("release-regressions", help="run mandatory declared P0 release regressions")
    ...
    if args.command == "project-health":
        from exilelens.ops.project_health import evaluate_project_health
    ...
    if args.command == "release-gate":
        from exilelens.ops.models import CheckResult, GateVerdict, Severity
        from exilelens.ops.release_gate import evaluate_release_gate
        from exilelens.ops.smoke import run_smoke
    ...
    if args.command == "release-regressions":
        from exilelens.ops.release_regressions import run_mandatory_release_regressions

        payload = run_mandatory_release_regressions(root=root)
        _print(payload, as_json=as_json)
        return 0 if payload["status"] == "PASS" else 2

    if args.command == "game-update":
        from exilelens.ops.game_update import run_game_update
    ...
    if args.command == "pob-compatibility":
        from exilelens.ops.pob_compat import inspect_pob_compatibility
    ...
    if args.command == "regression-audit":
        from exilelens.ops.regression import load_registry, select_for_changed_paths, select_for_subsystems, paths_for_entries
    ...
    if args.command == "market-health":
        from exilelens.ops.market_health import diagnose_offline
    ...
    if args.command == "build-source-audit":
        from exilelens.ops.build_source_audit import audit_build_source
    ...
    if args.command == "fixture-audit":
        from exilelens.ops.fixture_audit import audit_fixture_corpus
    ...
    if args.command == "release-notes":
        from exilelens.ops.release_notes import collect_release_notes
    ...
    if args.command == "watch-patch":
        from exilelens.ops.watchers import watch_patch

...
from exilelens._paths import is_frozen, repo_root
from exilelens.build_source import BuildSource, read_build_source
from exilelens.build_sources import (
    BuildRevision,
...
from exilelens.tooltip_perf import perf_enabled
from exilelens.errors import RestoreFailed, WorkerUnhealthy
from exilelens.worker import WorkerSession, decorate_build_result, decorate_evaluation, decorate_metrics
...
class SubprocessWorkerClient:
    ...
    def _command(self) -> tuple[list[str], dict[str, str]]:
        ...
        env["PYTHONUTF8"] = "1"
        if is_frozen():
            cmd = [sys.executable, "--exilelens-worker"]
        else:
            cmd = [sys.executable, "-X", "utf8", "-m", "exilelens.worker"]
    ...
    def request(self, method: str, params: dict[str, Any] | None = None, *, timeout_s: float | None = None) -> Any:
        ...
        if not response.get("ok"):
            from exilelens.errors import raise_from_payload
...
class Engine:
    ...
    def list_calculable_effects(
        ...
        *,
        ...
        """
        from exilelens.items.effect_components import normalize_effect_catalog
    ...
    def read_effect_metrics(
        ...
        *,
        ...
        zero extra frames.
        """
        from exilelens.items.effect_components import ComponentReference
        from exilelens.worker import decorate_contextual_effect_result
    ...
    def evaluate_item_slots(
        ...
        *,
        ...
        """
        from exilelens.worker import decorate_item_slot_evaluation
    ...
    def finalize_transaction(self) -> dict[str, Any]:
        ...
        """
        from exilelens.worker import decorate_restored_block
    ...
    def evaluate_slot_cleared(self, slot: str, *, context: str | None = None) -> dict[str, Any]:
        from exilelens.worker import decorate_slot_cleared
    ...
    def evaluate_gear_plan(
        ...
        *,
        ...
        from exilelens.worker import decorate_gear_plan_evaluation
    ...
    def evaluate_tree_path(
        ...
        *,
        ...
        from exilelens.tree.mutations import decorate_tree_evaluation
    ...
    def list_item_sets(self) -> dict[str, Any]:
        from exilelens.baseline import normalize_item_sets_payload
    ...
    def set_active_item_set(self, item_set_id: str | int) -> dict[str, Any]:
        from exilelens.baseline import item_set_wire_value

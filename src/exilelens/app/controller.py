...
from exilelens.app.item_check_lifecycle import (
    ITEM_CHECK_USER_TIMEOUT_MS,
    ITEM_CHECK_WATCHDOG_MS,
    ItemCheckLifecycleRegistry,
    ItemCheckPhase,
)
from exilelens.app.heartbeat import HeartbeatState, StallKind
from exilelens.app.runtime_trace import trace
from exilelens.app.scheduler import BoundedEvaluationScheduler
from exilelens.app.settings import AppSettings, BaselineMode, save_settings
from exilelens._version import __version__, git_commit
...
from exilelens.errors import EngineError, NotPoe2Item, UnsupportedGameLanguage, WorkerUnhealthy
from exilelens.items.cache import ItemPipelineCache
from exilelens.items.evaluation import classify_localization_failure, evaluate_item
from exilelens.items.language_detect import UNSUPPORTED_LANGUAGE_TITLE
from exilelens.items.history import baseline_identity
from exilelens.items.compare import STALE_BUILD_CHANGED, PinCompareState, build_compare_summary
from exilelens.items.item_check_settings import ItemCheckProSettings
from exilelens.items.loot_review import LootReviewSession
from exilelens.items.persistent_history import PersistentItemHistory
from exilelens.items.potential import UpgradePotentialAnalyzer
from exilelens.items.primary_metric import resolve_primary_metric
from exilelens.items.build_intel.decomposition import run_bounded_decomposition
from exilelens.items.build_intel.engine import attach_decomposition
from exilelens.items.build_intel.item_semantics import parse_item_semantics
from exilelens.items.build_intel.relevance import rank_groups_for_decomposition
from exilelens.items.build_intel.models import BuildMod
from exilelens.items.price import ManualPrice, ManualPriceError, parse_manual_price
from exilelens.items.evaluation_identity import EvaluationContextIdentity, EvaluationIdentity, identity_from_state
...
from exilelens.items.result_cache import EvaluationResultCache, evaluation_cache_key
from exilelens.items.upgrade_path import attach_upgrade_path_presentation
from exilelens.items.value_layer import rescore_evaluation
from exilelens.app.build_state import BaselineState, BuildInfo, BuildState
from exilelens.app.item_dismiss import ItemDismissController
...
from exilelens.app.price_check_session import PriceCheckSession
from exilelens.price_check.panel_edits import apply_panel_edits
from exilelens.app.price_check_hotkey import PriceCheckHotkeyController
from exilelens.app.refine_price_hotkey import RefinePriceHotkeyController
from exilelens.app.settings import DEFAULT_PRICE_CHECK_HOTKEY
...
from exilelens.market.models import MarketSearchRequest as DomainMarketSearchRequest
from exilelens.market.models import SearchDepth
from exilelens.gear.engine import run_gear_optimization
from exilelens.gear.cache import GearPlanEvalCache
from exilelens.gear.models import GearOptimizationRequest as DomainGearOptimizationRequest, GearSearchPreset, PlanConstraint
from exilelens.gear.registry import CandidatePoolRegistry
from exilelens.market_assist.evaluator import evaluate_capture_observation
from exilelens.market_assist.finalization import finalize_session_to_pool, pool_to_handoff_payload
...
from exilelens.market_assist.settings import MarketAssistantRuntimeSettings
from exilelens.market.models import ListingPrice
from exilelens.platform.windows.foreground_info import evaluate_poe_foreground_match
from exilelens.price_check.capture_diagnostics import CapturePhase, log_capture_phase
from exilelens.price_check.cache import PriceCheckCache
from exilelens.price_check.diagnostic_mode import (
    is_market_only_mode,
    load_market_fixture_text,
)
from exilelens.price_check.factory import build_default_price_check_providers, build_market_only_price_check_providers
from exilelens.price_check.league_catalog import LeagueCatalog
from exilelens.price_check.league_resolver import (
    MODE_PINNED,
    LeagueResolution,
    describe_resolution,
    resolve_market_league,
)
from exilelens.price_check.models import CompiledPriceCheckRequest, LeagueContext, PriceCheckRequest as DomainPriceCheckRequest
from exilelens.price_check.presentation import (
...
from exilelens.baseline import ResolvedBaseline, canonical_item_set_id, resolve_item_set
from exilelens.tree.calibration_session import CalibrationSession
from exilelens.tree.tracked_loader import TrackedTreeLoader
    ...
    def run_baseline_reload(self, request: BaselineReloadRequest) -> None:
        ...
        try:
            ...
            try:
                ...
                equipment = {}
            from exilelens.config import fingerprint_hash
            from exilelens.tree.fingerprint import tree_fingerprint_from_components
...
class EvaluationController(QObject):
    ...
    def _check_price_within_budget(self, domain_request):
        ...
        """
        from exilelens.price_check.network_withhold import STAGE_FETCH
        from exilelens.price_check.rate_policy import FETCH, SEARCH, shared_policy_registry
    ...
    def _neighbourhood_for(self, item_raw: str, league: str | None, *, hypothesis=None) -> str:
        ...
        from exilelens.price_check.market_drivers import neighbourhood_identity
        from exilelens.price_check.market_session import neighbourhood_fingerprint
    ...
    def _panel_model_for(
        ...
        from exilelens.price_check.panel_model import panel_model_for_result
    ...
    def submit_panel_refresh(self, edits: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not session.accepts(generation):
            ...
            return None
        from exilelens.price_check.panel_edits import apply_plan_edits
        from exilelens.price_check.market_plan import compile_plan
        ...
        try:
            ...
            from dataclasses import replace
            from exilelens.price_check.panel_model import PanelState
    ...
    def submit_refined_price_check(self, hypothesis) -> dict[str, Any] | None:
        ...
        try:
            ...
            if queued is not None:
                from exilelens.price_check.rate_policy import SEARCH, shared_policy_registry
    ...
    def set_value_profile(self, profile: str) -> dict[str, Any] | None:
        ...
        if self._last_analysis and self._analysis_generation == self._baseline_generation:
            if self._last_analysis.get("kind") == "tree":
                from exilelens.tree.ranking import rescore_tree_results
    ...
    def _spawn_pinned_overlay(self, entry) -> None:
        ...
        if saved:
            ...
            if anchor:
                from exilelens.ui.overlay_positioning import compute_overlay_placement

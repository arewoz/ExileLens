...
from exilelens.analysis.search_intent import build_search_intent
from exilelens.analysis.slot_compat import load_slot_compat, slot_compatibility
from exilelens.errors import EngineError
from exilelens.items.primary_metric import resolve_primary_metric
from exilelens.items.slots import EVALUABLE_POB_SLOTS, pob_slot_to_product
        ...
        if product_slot in SAFE_CONTRIBUTION_SLOTS:
            try:
                ...
                if stripped.strip() != item_raw.strip():
                    ...
                    from exilelens.analysis.probes import score_probe_metrics
...
def rescore_analysis(result: dict[str, Any], profile: str | ValueProfile) -> dict[str, Any]:
    ...
    def _rescore_probe(probe: dict[str, Any]) -> dict[str, Any]:
        ...
        if probe.get("baseline_raw") and probe.get("probed_raw"):
            ...
            from exilelens.items.value_profiles import score_profile
    ...
    from exilelens.analysis.audit import BuildNeed

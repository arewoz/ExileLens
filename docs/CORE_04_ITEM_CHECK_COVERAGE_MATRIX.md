...
| Jewel (connectivity-affecting, e.g. "From Nothing"-style "Passives in Radius can be Allocated without being connected") | UNSUPPORTED BY PRODUCT (bounded) | A currently-equipped jewel of this family is excluded from the compatible-socket list (`excluded_connectivity_risky_socket_count`) when PoB's own `item.jewelData.intuitiveLeapLike` flag is set. One corpus fixture (`core04_skill_native_dot.xml`, socket "Jewel 26196") carries a sanitized/relabeled jewel that reproduces the same underlying restore risk (temporarily replacing it deallocates dependent passives that a plain `SetSelItemId` restore does not reinstate) without PoB recognizing it as the flagged mechanic, so the filter alone does not cover it — the existing transaction restore-verification (`tree_nodes` fingerprint check) independently catches it and safely fails the evaluation (`RESTORE_FAILED` → `EvaluationInvalidBuildState`, engine invalidated, no wrong result ever delivered). Bounded, known limitation; not corruption. |

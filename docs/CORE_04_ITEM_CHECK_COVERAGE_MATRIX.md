...
| Weapon-set swap / `Weapon 2 Swap` | COVERED | `core04_weapon_swap.xml` covers weapon-half substitution (`test_weapon_swap_candidate_substitution_resolves_the_active_slot`, M1.1) and, as of M1.2, offhand-half substitution (`test_weapon_swap_offhand_candidate_substitution_resolves_the_active_slot`): a Quiver candidate targeting logical `Weapon 2` correctly resolves against the ACTIVE `Weapon 2 Swap` item via the same `active_weapon_slot` bridge translation, with no separate offhand-swap mapper. `ProductSlot.OFFHAND_2` remains unreachable from live data (see `src/exilelens/items/slots.py`); this was re-confirmed, not re-litigated, during the M1.2 audit. |
| Weapon-set-qualified component contexts (Slice 3, internal) | COVERED | `tests/integration/test_weapon_set_component_contexts.py` (real PoB, explicit-only): the same `ComponentReference` reads under set 1 and set 2 with disjoint cache observations (`EscapeShotPlayer`: set 2 `MEASURED`, CombinedDPS ~1763.7; set 1 `MEASURED` with PoB-native ~zero output from a distinct `GlobalCache` entry -- truthful context-specific availability, never a synthetic zero); physical `Weapon 1 Swap` candidate measures a real +121%-class `CombinedDPS` component delta with the opposite set byte-identical; physical set-1 placement leaves set 2 identical; invalid set / slot-set mismatch / failed-candidate / corrupt-restore all fail closed with exact restore; repeated set2->set1->set2 reads are deterministic; ordinary ring Item Check invokes zero contextual RPCs and keeps `FULL`/`MEANINGFUL_UPGRADE`. Contextual deltas are component evidence only and never enter the public verdict path. |
...
| Flask | UNSUPPORTED BY PRODUCT | Terminal `ITEM_UNSUPPORTED`, not scored as equipment. |
| Jewel (normal, per-socket) / dynamic `"Jewel <nodeId>"` sockets | COVERED (M1.3; discovery correctness fixed P1.1b) | Jewel sockets are tree-node-keyed, not fixed equipment slots (`spec.jewels[nodeId]`, see `docs/POB2_ENGINE_CONTRACT.md`'s Jewel section) — not modeled as a `ProductSlot`. `core04_melee_weapon.xml` (5 allocated, occupied sockets: a Timeless Jewel, two unique Diamonds, two rare Rubies) proves occupied-socket replacement with a real measurable delta, multi-socket ranking that selects the best of several materially different placements, a real multi-axis `TRADEOFF`, clean restore, and stable repeated evaluation. **P1.1b correction:** `core04_skill_native_dot.xml` was previously documented as "19 allocated sockets, 15 empty" — this was wrong. Ground truth is 4 allocated sockets (7960/21984/26196/61419), all occupied; the 19 figure was every Socket-type tree node PoB's `ItemsTab` creates across the whole passive tree, never filtered by `spec.allocNodes`, because the headless bridge never called the one PoB function (`ItemsTabClass:UpdateSockets`) that computes the allocation-derived `.inactive` flag it relied on. Fixed in `allocated_jewel_socket_slots()` (`runtime/lua/bridge.lua`), which now calls `UpdateSockets()` explicitly before reading it. No public-corpus fixture currently has a genuinely allocated-but-empty jewel socket (checked across all nine builds) — empty-socket inclusion is covered by code-path argument (the discovery filter has no occupancy check at all), not a real-engine fixture; see `tests/integration/test_jewel_real_pob.py`'s module docstring and `test_empty_allocated_socket_is_compared_against_no_jewel` (currently skipped with rationale). |
| Jewel (cluster) | NOT APPLICABLE | No cluster-jewel base type exists in the currently supported PoE2 PoB data (`Data/Bases/jewel.lua`); `IsItemValidForSlot`'s cluster-jewel branch is unreachable for the currently supported revision. Documented, not fabricated. |
| Jewel (connectivity-affecting, "Intuitive Leap"-like, e.g. "From Nothing") | COVERED (M1.3 remediation slice) | Root cause found and fixed: `repair_tree_allocation` (`runtime/lua/bridge.lua`) restores `spec.allocNodes`/`spec.hashOverrides`/`spec.masterySelections` to the true baseline after every slot revert, mirroring PoB's own `ItemsTabClass:DeleteItem` logic in reverse. `core04_mixed_hit_ailment.xml`, `core04_minion_actor.xml`, and `core04_skill_native_dot.xml` (all carrying a "From Nothing"-equivalent jewel) now restore correctly and evaluate a full multi-socket batch with zero `RESTORE_FAILED`. See `docs/POB2_ENGINE_CONTRACT.md`'s restore-safety section. |
| Jewel (alternate-start-point, "Split Personality"-style "Can Allocate Passive Skills from the &lt;Class&gt;'s starting point") | UNSUPPORTED BY PRODUCT (bounded) | The primary offense metric restores correctly after the fix above, but a small secondary-metric discrepancy (`Life`, ~2.5%) remains unresolved (root cause partially understood; `PassiveSpecClass:BuildAllDependsAndPaths()` tested directly and ruled out as the missing piece). This socket is excluded from discovery (`item.jewelData.alternateClassStart`, `excluded_connectivity_risky_socket_count`) rather than risking the small wrong value. One corpus fixture (`core04_stage_context.xml`, socket "Jewel 54127"). |
| Jewel (sinister sockets / ascendancy-embedded sockets, e.g. Lich) | NOT COVERED (architecture-verified, no fixture) | `ItemsTabClass:IsItemValidForSlot` (reused unmodified by the M1.3 jewel-socket resolver) enforces non-unique-only restrictions for these socket types, but no corpus fixture currently allocates one; not fabricated per Phase spec. |

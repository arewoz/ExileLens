...
| Jewel (normal, per-socket) / dynamic `"Jewel <nodeId>"` sockets | COVERED (M1.3) | Jewel sockets are tree-node-keyed, not fixed equipment slots (`spec.jewels[nodeId]`, see `docs/POB2_ENGINE_CONTRACT.md`'s Jewel section) — not modeled as a `ProductSlot`. `core04_melee_weapon.xml` (5 allocated, occupied sockets: a Timeless Jewel, two unique Diamonds, two rare Rubies) proves occupied-socket replacement with a real measurable delta, multi-socket ranking that selects the best of several materially different placements, a real multi-axis `TRADEOFF`, clean restore, and stable repeated evaluation. `core04_skill_native_dot.xml` (19 allocated sockets, 15 empty) proves empty-allocated-socket comparison (candidate vs no jewel, never inferred as replacing a neighboring socket). See `tests/integration/test_jewel_real_pob.py`. |

...
3. Add a public fixture for the skill-native-DoT branch (`DamageQuantity.SKILL_DOT`, e.g. a Bleed/Essence-Drain-style build with no separate "ailment" field). The mixed hit+ailment branch (`DamageQuantity.HIT_PLUS_AILMENT`) is now covered by `core04_mixed_hit_ailment.xml` (M1.1). An earlier candidate for that branch — a lower-DPS Huntress/Ritualist Poisonburst Arrow character where PoisonDPS is ~96.5% of hit rather than dominant — was found but not used because its active loadout uses `useSecondWeaponSet="true"` (the true equipped weapons are in the "Weapon 1 Swap"/"Weapon 2 Swap" slots); that remains a real candidate for a future weapon-swap-active-set coverage slice, not for offense-quantity coverage.
